...
/**
 * AccountNotFound — displayed when a customer or supplier account lookup fails.
 *
 * Props:
 *  @param {string} accountType   - 'Customer' | 'Supplier'  (default: 'Account')
 *  @param {string} accountId     - The slug / ID that was looked up (optional)
 *  @param {string} backPath      - Where the back button navigates (default: '/accounts/customers')
...
const AccountNotFound = ({
  ...
  const isSupplier = accountType.toLowerCase() === 'supplier';
    ...
    <div className="min-h-screen bg-[#F7F9FB] flex flex-col">
      ...
      <header className="bg-[#F7F9FB] flex items-center gap-4 px-8 py-5 border-b border-[#ECEEF0]">
        ...
        >
          ...
          <span className="text-sm font-medium">Back to {accountType} Accounts</span>
        </button>
        <div className="bg-[#ECEEF0] h-6 w-[1px]" />
        <h1 className="text-lg font-bold text-[#191C1E]">{accountType} Not Found</h1>
      </header>
      ...
      <main className="flex-grow flex items-center justify-center px-6 py-24 relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-0 pointer-events-none opacity-20">
          <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid slice" viewBox="0 0 1000 1000">
            <defs>
              <linearGradient id="gradNF" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <circle cx="500" cy="500" r="400" fill="url(#gradNF)" />
          </svg>
        </div>
        ...
        <div className="max-w-lg w-full text-center relative z-10 flex flex-col items-center">
          ...
          <div className="relative mb-8">
            ...
            <div className="relative w-24 h-24 rounded-full bg-white shadow-lg border border-[#C3C6D7]/20 flex items-center justify-center">
              <SearchX
                size={40}
                className={isSupplier ? 'text-emerald-500' : 'text-[#2563EB]'}
                strokeWidth={1.5}
              />
            </div>
          </div>
          ...
          <p className="text-[#434655] text-sm max-w-sm mx-auto leading-relaxed mb-2">
            {error
              ? error
              : `We couldn't find a ${accountType.toLowerCase()} account matching this record. It may have been deleted or the link is incorrect.`}
          </p>
          ...
          {accountId && (
            <div className="mt-3 mb-8 inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#FFF3CD] border border-[#FFC107]/30 rounded-lg">
              ...
              <span className="text-xs font-semibold text-amber-800 font-mono">{accountId}</span>
            </div>
          )}
