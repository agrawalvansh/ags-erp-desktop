...
const BuyerAccount = () => {
  ...
  const filteredBuyers = useMemo(() => {
    ...
    return buyers.filter((u) =>
      ...
      String(u.address || '').toLowerCase().includes(term) ||
      String(u.company || '').toLowerCase().includes(term) ||
      String(u.gstNumber || '').toLowerCase().includes(term) ||
  ...
  }, [filteredBuyers, sortConfig, currentPage]);
  ...
  const handleDeleteClick = async (e, buyer) => {
    e.stopPropagation();
    setDeleteError(null);
    try {
      ...
      if (deps.hasDependencies) {
        ...
        setDeleteError(
          parts.length > 0
            ? `Cannot delete: this customer has ${parts.join(', ')} entries. Remove them first.`
            : 'Cannot delete: this customer has dependent entries. Remove them first.'
        );
  ...
  const confirmDeleteCustomer = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await window.api.invoke('customers:delete', deleteTarget.id);
      if (result.success) {
      ...
      <div className="px-4 md:px-8 pt-8 pb-2">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            <div>
              <h1 className="text-3xl font-extrabold tracking-tight text-[#191C1E] mb-1">Customer's Accounts</h1>
              <p className="text-[#434655] text-sm font-medium">Showing {filteredCount} customers</p>
            </div>
                ...
                {processedBuyers.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="py-5 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-semibold text-[#191C1E]">{buyer.name}</span>
                        </div>
                      </td>
                      <td className="py-5 px-6 text-sm text-[#434655] font-medium">{buyer.phone}</td>
                      <td className="py-5 px-6 text-sm text-[#434655]">{buyer.address}</td>
                      <td className="py-5 px-6 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={(e) => handleDeleteClick(e, buyer)}
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-12 text-center text-[#434655] text-sm">
                      {loadingBuyers ? 'Loading...' : 'No customers found.'}
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {totalPages > 1 && (
            <div className="px-8 py-6 flex items-center justify-between bg-[#F2F4F6]/30 border-t border-[#C3C6D7]/10">
              <p className="text-sm text-[#434655]">
                ...
                of <span className="font-bold text-[#191C1E]">{filteredCount}</span> customers
              </p>
