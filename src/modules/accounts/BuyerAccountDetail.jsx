...
const BuyerAccountDetail = () => {
  ...
  const accountData = useMemo(() => {
    const maalEntries = invoices.map((inv) => {
      ...
      const computedSource = inv.source ? inv.source : (isLinkedToOrder ? 'order' : 'invoice');
      const isLinkedToInvoice = computedSource === 'invoice';
      return {
        type: 'maal',
        id: `M-${inv.invoice_id}`,
  ...
  const handleInvoiceNavigate = (invoiceId) => {
    if (invoiceId && (invoiceId.startsWith('E-') || invoiceId.startsWith('AGS-I-'))) {
      navigate(`/invoice/${invoiceId}`, { state: { fromAccount: `/accounts/customers/${slug}` } });
    }
  };
  ...
  const generateLedgerPDF = () => {
    ...
    doc.text('Customer Ledger', margin, 15);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...darkGray);
    doc.text(`Customer: ${buyer.name || '—'}`, margin, 22);
    doc.text(`ID: ${slug}`, margin, 27);
    if (buyer.mobile) doc.text(`Mobile: ${buyer.mobile}`, margin + 90, 22);
    if (buyer.address) doc.text(`Address: ${buyer.address.substring(0, 70)}`, margin + 90, 27);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...black);
    const balanceLabel = grandTotal > 0 ? 'Balance Receivable' : 'Balance Clear';
    ...
    const safeName = (buyer.name || slug || '').replace(/[^a-zA-Z0-9 ]/g, '').trim();
    ...
    <div className="min-h-screen bg-[#F7F9FB]">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-[100] print:hidden" style={{ background: 'rgba(255, 255, 255, 0.7)', backdropFilter: 'blur(8px)' }}>
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md mx-4 w-full">
            ...
            <p className="text-[#64748B] text-center mb-4 text-sm">
              {pendingPDFData?.fileName || 'Customer Ledger'}
            </p>
            <div className="mb-6">
      ...
      <header className="w-full h-16 sticky top-0 z-40 bg-[#F7F9FB] flex items-center justify-between px-8 border-b border-[#C3C6D7]/10">
        <div className="flex items-center gap-4">
          <button
            onClick={() => navigate('/accounts/customers', { state: { returnedFromAccount: slug } })}
                      ...
                      <td className="px-4 py-3 text-center">
                        {editingRow === row.id ? (
                          ...
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <button className="p-1.5 rounded-full hover:bg-white text-[#434655] hover:text-[#DC2626] transition-all hover:shadow-sm cursor-pointer" onClick={() => setDeleteTarget(row)} title={row.isLinkedToInvoice ? 'Delete Invoice' : 'Delete Entry'}><Trash2 size={16} /></button>
                          </div>
                        )}
                      </td>
                    </tr>
        ...
        >
          <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border border-[#C3C6D7]/20 p-8">
            ...
            <h2 id="delete-entry-heading" className="text-2xl font-extrabold text-[#0F172A] tracking-tight mb-3">
              {deleteTarget?.type === 'maal' && deleteTarget?.isLinkedToInvoice ? 'Delete Invoice?' :
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6 space-y-1">
              {maalData.length > 0 && (
                <p className="text-sm text-red-700">
                  <span className="font-bold">{maalData.length}</span> maal entr{maalData.length === 1 ? 'y' : 'ies'}</p>
              )}
              {jamaData.length > 0 && (
